...
**Fase 4 construida completa** ⏳ (en validación: 60 días de paper desde el 17-08-2026) — ejecución real en Alpaca paper con reconciliación previa (divergencia → HALT), idempotencia de mercado, y ahora también:

- **Ventas defensivas deterministas** (`python -m app.monitor`, cada 15 min con mercado abierto): stop duro −20%, invalidadores de tesis medibles, horizonte cumplido y recorte por concentración — en ese orden de precedencia, sin consultar jamás a la IA. También corren en el ciclo diario antes de decidir compras.
- **Dead man's switch**: el monitor late cada 15 min; al primer latido queda armado, y si deja de latir >45 min, las compras nuevas se bloquean solas (kill switch global) con alerta.
- **Dashboard web** en `localhost:8000/`: curva de equity, posiciones con P&L, tesis abiertas con invalidadores, decisiones de IA, órdenes, reconciliaciones y estado de kill switch/DMS.

**Fase 2 → motor point-in-time listo** — el backtest acepta máscara de elegibilidad sin look-ahead: constituyentes históricos (`python -m app.jobs universe-load archivo.csv`, formato `symbol,valid_from,valid_to`) y filtro de calidad por fundamentales SEC EDGAR con **fecha de publicación real** (`python -m app.jobs fundamentals` — configura `SEC_USER_AGENT` en `.env`). Con fundamentales cargados, el backtest excluye automáticamente empresas cuyo crecimiento de ingresos *publicado a esa fecha* era negativo. Falta solo conseguir el CSV de constituyentes históricos para cerrar el survivorship bias. 55 tests en verde.
